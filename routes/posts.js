...
const { PostsModel, validPost } = require("../models/postModel");
...
router.put('/deleteWait/:postId/:userId',async (req, res) => {
    try {
        ...
        const userIndex = post.waitingList.indexOf(userId);
        if (userIndex === -1) {
            return res.status(404).json({ message: "user not found in the waitings list" });
        }

        post.waitingList.splice(userIndex, 1); // Remove post from waitings list
        await post.save();
