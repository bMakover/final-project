...
const MyDemandsList = () => {
  ...
  return (
    <div>
      <h2>My Demands List</h2>
      <ul>
        {myDemands.map((demand) => (
          <li key={demand._id}>
            <strong>Source:</strong> {demand.source.city}, {demand.source.street}, {demand.source.houseNumber}<br />
            <strong>Destination:</strong> {demand.destination.city}, {demand.destination.street}, {demand.destination.houseNumber}<br />
            <strong>Limit Date:</strong> {new Date(demand.limitDate).toLocaleDateString()}<br />
            {/* Add other demand details as needed */}
            <button onClick={() => handleDeleteDemand(demand._id)}>Delete</button>
          </li>
        ))}
      </ul>
    </div>
