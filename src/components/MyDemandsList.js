...
const MyDemandsList = () => {
  ...
  return (
    <div className='container row col-3 align-items-start'>
      <ul>
        {myDemands.map((demand) => (
          <li className='border m-2 w-80 p-2 shadow card' key={demand._id}>
            ...
            <button className="button-56 m-3" onClick={() => handleDeleteDemand(demand._id)}> מחק בקשה זו <i className="fa fa-trash m-1" aria-hidden="true"></i></button>
          </li>
        ))}
      </ul>
    </div>
